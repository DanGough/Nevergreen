try {
    ...
    $Product = ($Response.data.contentJson.entries | ConvertFrom-Json).downloadData.'camera-hub-win'
    ...
    New-NevergreenApp -Name 'Elgato Camera Hub' -Version $Product.version -Uri $Product.downloadURL -Architecture 'x64' -Type 'MSI'
}
catch {
